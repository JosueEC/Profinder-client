import { useEffect } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { getAllSuppliers } from "../../services/redux/actions/actions";

import { Box, Container, Divider, Flex, Heading, Image, Stack, Text, useColorModeValue, Wrap, ScaleFade } from "@chakra-ui/react";

import {
  FaUserAlt,
  FaRegPaperPlane,
  FaMailBulk,
  FaPhone,
  FaMapMarkerAlt,
} from "react-icons/fa";
import NoAvatar from "../../assets/defaultImages/sinfoto.webp";
import Tag from "../../singleComponents/Tag";
import InfoLabel from "../../singleComponents/InfoLabel";
import SupplierPost from "../../components/SupplierPost/SupplierPost";

const ArticleList = () => {
  const suppliers = useSelector((state) => state.suppliers);
  const dispatch = useDispatch();
console.log(suppliers)
  useEffect(() => {
    dispatch(getAllSuppliers());
  }, [dispatch]);

  return (
    <Container
      color="gray.300"
      bg={useColorModeValue("gray.800", "gray.800")}
      maxW="100%"
      py="5"
      px={{ base: "8", md: "8", lg: "10rem" }}
      key={suppliers?.id}
    >
      {suppliers?.length > 0 && (
        <Heading as="h1">{suppliers?.id || "Sin nombre"}</Heading>
      )}
      <ScaleFade initialScale={0.9} in>
        <Box
          p={7}
          bg="blackAlpha.800"
          marginTop={{ base: "1", sm: "5" }}
          display="flex"
          flexDirection={{ base: "column", sm: "row" }}
          justifyContent="space-between"
        >
          <Box
            display="flex"
            flex="1"
            marginRight="3"
            position="relative"
            alignItems="center"
          >
            <Box
              width={{ base: "100%", sm: "85%" }}
              zIndex="2"
              marginLeft={{ base: "0", sm: "5%" }}
              marginTop="5%"
            >
              <Image
                borderRadius="50%"
                boxSize="300px"
                src={suppliers.image}
                fallbackSrc={NoAvatar}
                loading="lazy"
                alt="avatar supplier"
                objectFit="contain"
              />
            </Box>
            <Box zIndex="1" width="100%" position="absolute" height="100%">
              <Box
                bgGradient="radial(teal.300 1px, transparent 1px)"
                backgroundSize="20px 20px"
                opacity="0.4"
                height="100%"
              />
            </Box>
          </Box>
          <Box
            display="flex"
            flex="1"
            flexDirection="column"
            justifyContent="center"
            marginTop={{ base: "3", sm: "0" }}
          >
            <Stack align="center" justify="center" direction="row" my={4}>
              {suppliers.professions &&
                suppliers.professions.map(({ ocupations }) =>
                  ocupations.map(({ id, name }) => (
                    <Tag key={id} textTag={name} />
                  ))
                )}
            </Stack>
            <Text as="p" marginTop="2" color="gray.500" fontSize="lg" mb={5}>
              {suppliers.description}
            </Text>
            <InfoLabel textLabel={suppliers.genre} iconLabel={FaUserAlt} />
            <InfoLabel
              textLabel={suppliers.years_exp}
              iconLabel={FaRegPaperPlane}
            />
            <InfoLabel textLabel={suppliers.email} iconLabel={FaMailBulk} />
            <InfoLabel textLabel={suppliers.phone} iconLabel={FaPhone} />
            <InfoLabel
              textLabel={suppliers.ubication}
              iconLabel={FaMapMarkerAlt}
            />
          </Box>
        </Box>
      </ScaleFade>

      <Flex
        align="center"
        gap={{ md: "3rem", lg: "3rem" }}
        direction={{ base: "column", md: "row", lg: "row" }}
      >
        <Heading as="h2" marginTop="5">
          Trabajos Recientes
        </Heading>
      </Flex>
      <Divider marginTop="5" />
      <Wrap spacing="30px" marginTop="5" justify="center">
        {suppliers.posts ? (
          suppliers.posts.map(({ image, content, title }) => (
            <SupplierPost
              key={content}
              imagePost={image}
              titularPost={title}
              descriptionPost={content}
            />
          ))
        ) : (
          <Heading>No hay ninguna publicación</Heading>
        )}
      </Wrap>
    </Container>
  );
};

export default ArticleList;
